...
def test_rules_parser_extracts_scene_shot_and_frame_range() -> None:
    req = "Make scene_001 shot_001 more cinematic; regenerate only frames 120-220 and set cfg 7.2"
    plan = plan_from_rules(req, {"project": "feature_film_demo", "scene": None, "shot": None}, dry_run=True)

    assert plan.context["scene"] == "scene_001"
    assert plan.context["shot"] == "shot_001"
